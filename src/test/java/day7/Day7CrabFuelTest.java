...
public class Day7CrabFuelTest {
	...
	private int alignToPosition(List<Integer> test, int key, Function<Integer, Integer> fuelCalculator) {
		return test.stream().map(crab -> fuelCalculator.apply(Math.abs(crab - key))).reduce(0, Integer::sum);
	}

	private int calculateFuel(int steps) {
		int cost = 0;
